...
if [[ -z "${BOOTSTRAP_JDK}" ]] ; then
    if [[ -d /AppleInternal/Java/Home/15 ]] ; then
        BOOTSTRAP_JDK=/AppleInternal/Java/Home/15
    elif [[ -d /AppleInternal/Java/Home/14 ]] ; then
        BOOTSTRAP_JDK=/AppleInternal/Java/Home/14
    elif [[ -d "$(/usr/libexec/java_home -v 15)" ]] ; then
        BOOTSTRAP_JDK=$(/usr/libexec/java_home -v 15)
    elif [[ -d "$(/usr/libexec/java_home -v 14)" ]] ; then
        BOOTSTRAP_JDK=$(/usr/libexec/java_home -v 14)
    fi
fi
